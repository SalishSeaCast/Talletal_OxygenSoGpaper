...
CONTAINS
   ...
   SUBROUTINE p4z_rem( kt, knt )
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
                ...
                ! Remineralization:
                !
                ! Bacterial oxidation of NH3 to NO pool proportional to NH
                ! 202108: added light dependence from Fennel 2006; based on PAR
                zz_NH_oxid = zz_remin_NH * zz_NH * tgfunc(ji,jj,jk) * (1._wp - MAX( (epar(ji,jj,jk) -zz_I0nit) / &
                ...
                tra(ji,jj,jk,jpnh4) = tra(ji,jj,jk,jpnh4) + (zz_remin_NH_DON - zz_NH_oxid) 
                tra(ji,jj,jk,jpdon) = tra(ji,jj,jk,jpdon) - zz_remin_NH_DON + zz_remin_NH_PON
                tra(ji,jj,jk,jppon) = tra(ji,jj,jk,jppon) - zz_remin_NH_PON
                z3d0(ji,jj,jk)= -zz_o2_nitr*zz_NH_oxid
#if defined key_oxy
                tra(ji,jj,jk,jpo2) = tra(ji,jj,jk,jpo2) - zz_o2_nitr*zz_NH_oxid - zz_o2_proreg * zz_remin_NH_DON
#endif
#if defined key_skog
                tra(ji,jj,jk,jpdic) = tra(ji,jj,jk,jpdic) + (zz_remin_NH_DON) * zz_redfield_c_n 
                tra(ji,jj,jk,jpta) = tra(ji,jj,jk,jpta) - (zz_redfield_p_n - 1.0) * (zz_remin_NH_DON) &
                        - 2 * zz_NH_oxid
#endif
            END DO
         END DO
      ...
      IF( l_trdtrc ) THEN
         ...
         END DO
         IF( iom_use("O2REM")) THEN
            CALL iom_put("O2REM",tra(:,:,:,jpo2)-ztrtra(:,:,:,jpo2))
         ENDIF
