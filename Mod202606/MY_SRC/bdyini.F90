MODULE bdyini
   ...
   INTEGER, PARAMETER          :: nrimmax = 25 ! maximum rimwidth in structured
...
CONTAINS
   ...
   SUBROUTINE bdy_init
      ...
      NAMELIST/nambdy/ nb_bdy, ln_coords_file, cn_coords_file,                 &
         ...
         &             rn_ssh_offset, cn_dyn3d, nn_dyn3d_dta, cn_tra, nn_tra_dta, &  
         &             ln_tra_dmp, ln_dyn3d_dmp, rn_time_dmp, rn_time_dmp_out, &
         &             rn_max_sponge, cn_ice_lim, nn_ice_lim_dta,              &
      ...
      ln_dyn3d_dmp(:) = .FALSE.
      rn_time_dmp(:) = 0.0_wp
      rn_time_dmp_out(:) = 0.0_wp
      rn_max_sponge(:) = 0.0_wp
      cn_ice_lim(:) = ''
      nn_ice_lim_dta(:) = 0
      rn_ice_tem(:) = 0.0_wp
      rn_ice_sal(:) = 0.0_wp
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambdy in configuration namelist', lwp )
      ...
      DO ib_bdy = 1,nb_bdy
        ...
        SELECT CASE( cn_dyn2d(ib_bdy) )                  
          ...
          CASE('flather')      
             ...
             dta_bdy(ib_bdy)%ll_v2d = .true.
          CASE('ssh')
             IF(lwp) WRITE(numout,*) '      Impose SSH at boundary'
             dta_bdy(ib_bdy)%ll_ssh = .true.
             dta_bdy(ib_bdy)%ll_u2d = .false.
             dta_bdy(ib_bdy)%ll_v2d = .false.
          CASE('orlanski')     
        ...
        IF( cn_dyn2d(ib_bdy) /= 'none' ) THEN
           ...
           ENDIF
        ENDIF
        IF(lwp) WRITE(numout,*) 'SSH offset:  ', rn_ssh_offset(ib_bdy)
        IF(lwp) WRITE(numout,*)
        ...
        SELECT CASE( cn_dyn3d(ib_bdy) )                  
          ...
          CASE('orlanski_npo')
             ...
             dta_bdy(ib_bdy)%ll_v3d = .true.
          CASE('sponge')
             IF(lwp) WRITE(numout,*) '      Sponge condition'
             dta_bdy(ib_bdy)%ll_u3d = .false.
             dta_bdy(ib_bdy)%ll_v3d = .false.
          CASE('orlanski_w_sponge')
             IF(lwp) WRITE(numout,*) '     Orlanski (fully oblique) with adaptive nudging and sponge'
             dta_bdy(ib_bdy)%ll_u3d = .true.
             dta_bdy(ib_bdy)%ll_v3d = .true.
	...
	IF ( cn_dyn3d(ib_bdy) == 'sponge' .or. cn_dyn3d(ib_bdy) == 'orlanski_w_sponge') THEN
#if ! defined key_dynldf_c3d
#if ! defined key_dynldf_c2d
              CALL ctl_stop &
                 & ( 'bdyini: Can not use sponge layer without key_ldf_c2d or key_ldf_c3d')
#endif
#endif
              IF(lwp) WRITE(numout,*) '      + high viscosity sponge zone with '
              IF(lwp) WRITE(numout,*) '      maximum viscosity: ',rn_max_sponge(ib_bdy),' m2/s'
        ENDIF
