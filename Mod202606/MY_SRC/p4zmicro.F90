...
CONTAINS
   ...
   SUBROUTINE p4z_micro( kt, knt )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) ::  zz_P_diat, zz_P_myri, zz_P_flag, zz_Z, zz_D_PON, zz_uZoo_mort_diat, &
        zz_uZoo_mort_myri, zz_uZoo_mort_flag, zz_uZoo_graz_PON, zz_uZoo_graz_Z, zz_uZoo_NatMort_Excr
#if defined key_trdtrc
      ...
      CALL wrk_alloc( jpi, jpj, jpk, zz_uZoo_NatMort_Excr)
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
                ...
                zz_Excr_uzoo = zz_rate_uzoo_excr * tgfunc(ji,jj,jk) * zz_Z(ji,jj,jk)
		          zz_uZoo_NatMort_Excr(ji,jj,jk) = zz_NatMort_uzoo + zz_Excr_uzoo
      ...
      CALL iom_put( "MORTMICZ", zz_uzoo_NatMort_Excr)
      ...
      IF( l_trdtrc ) THEN
         ...
         END DO
         IF( iom_use("O2MIZ")) THEN
            CALL iom_put("O2MIZ",tra(:,:,:,jpo2)-ztrtra(:,:,:,jpo2))
         ENDIF
      ...
      CALL wrk_dealloc( jpi, jpj, jpk, zz_uZoo_NatMort_Excr)
      !
