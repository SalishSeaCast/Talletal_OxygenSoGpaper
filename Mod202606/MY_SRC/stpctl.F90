MODULE stpctl
   ...
   PUBLIC stp_ctl_check_negative_depth
#endif
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE stp_ctl( kt, kindic )
      ...
      IF( lk_dynspg_ts .and. kindic == -4 )  THEN
         sshn(:,:) = ssha_e(:,:) ! required for making sure that ssh at the time of the crash is found in output.abort
      ENDIF
...
9300  FORMAT(' it :', i8, ' ssh2: ', e16.10, ' Umax: ',e16.10,' Smin: ',e16.10)
   ...
   !!----------------------------------------------------------------------
   !!----------------------------------------------------------------------
   SUBROUTINE stp_ctl_check_negative_depth(kt,kindic)
   ! arguments
   INTEGER, INTENT( in )    ::    kt
   INTEGER, INTENT( inout ) ::   kindic  ! indicator of solver convergence
   ! locals
   REAL(wp) :: zumax, zvmax, ztmax, z_depth
   INTEGER :: ii1, ij1, ii2, ij2, ii3, ij3, ilocu(2)
   REAL(wp), DIMENSION(jpi,jpj) :: zht_e
   LOGICAL,  DIMENSION(jpi,jpj) :: lmask
   !!----------------------------------------------------------------------

   zht_e(:,:) = ht_0(:,:) + ssha_e(:,:)
   lmask(:,:) = .false.; where ( umask(:,:,1) > 0.5_wp ) lmask(:,:) = .true.; zumax = MINVAL( hu_e(:,:), lmask )
   lmask(:,:) = .false.; where ( vmask(:,:,1) > 0.5_wp ) lmask(:,:) = .true.; zvmax = MINVAL( hv_e(:,:), lmask )
   lmask(:,:) = .false.; where ( tmask(:,:,1) > 0.5_wp ) lmask(:,:) = .true.; ztmax = MINVAL(zht_e(:,:), lmask )
   z_depth = MIN( zumax, zvmax, ztmax )
   IF( lk_mpp )   CALL mpp_min( z_depth )   ! min over the global domain
   IF ( z_depth < 0.d0 ) THEN
      IF( lk_mpp ) THEN
         CALL mpp_minloc( hu_e,umask(:,:,1),zumax,ii1,ij1)
         CALL mpp_minloc( hv_e,vmask(:,:,1),zvmax,ii2,ij2)
         CALL mpp_minloc(zht_e,tmask(:,:,1),ztmax,ii3,ij3)
      ELSE
         lmask(:,:) = .false.; where ( umask(:,:,1) > 0.5_wp ) lmask(:,:) = .true.; ilocu = MINLOC( hu_e(:,:), lmask )
         ii1 = ilocu(1) + nimpp - 1
         ij1 = ilocu(2) + njmpp - 1
         lmask(:,:) = .false.; where ( vmask(:,:,1) > 0.5_wp ) lmask(:,:) = .true.; ilocu = MINLOC( hv_e(:,:), lmask )
         ii2 = ilocu(1) + nimpp - 1
         ij2 = ilocu(2) + njmpp - 1
         lmask(:,:) = .false.; where ( tmask(:,:,1) > 0.5_wp ) lmask(:,:) = .true.; ilocu = MINLOC(zht_e(:,:), lmask )
         ii3 = ilocu(1) + nimpp - 1
         ij3 = ilocu(2) + njmpp - 1
      ENDIF
      IF(lwp) THEN
         WRITE(numout,cform_err)
         WRITE(numout,*) ' ====== '
         WRITE(numout,*) ' a negative depth has been detected inside the domain'
         WRITE(numout,*) ' ====== '
         WRITE(numout,*) ' check: the U-depth'
         WRITE(numout,*) ' ====== '
         WRITE(numout,9500) kt, zumax, ii1, ij1
         WRITE(numout,*)
         WRITE(numout,*) ' check: the V-depth'
         WRITE(numout,*) ' ====== '
         WRITE(numout,9500) kt, zvmax, ii2, ij2
         WRITE(numout,*)
         WRITE(numout,*) ' check: the T-depth'
         WRITE(numout,*) ' ====== '
         WRITE(numout,9500) kt, ztmax, ii3, ij3
         WRITE(numout,*)
         WRITE(numout,*) '          output of last fields in numwso'
      ENDIF
      kindic = -4
...
9500  FORMAT (' kt=',i6,' min depth: ',1pg11.4,', i j: ',2i5)
   END SUBROUTINE stp_ctl_check_negative_depth
#endif
