...
CONTAINS
   ...
   SUBROUTINE tra_adv_tvd_zts_bio ( kt, kit000, cdtype, p2dt, pun, pvn, pwn0,      &
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) :: ztrdx, ztrdy, ztrdz
      REAL(wp), POINTER, DIMENSION(:,:,:,:) :: ztrs
      REAL(wp), POINTER, DIMENSION(:,:,:) :: advx, advy, advz
      !!----------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('tra_adv_tvd_zts')
      !
      ...
      !
      IF( l_trd )  THEN
         CALL wrk_alloc( jpi, jpj, jpk, ztrdx, ztrdy, ztrdz, advx, advy, advz )
      ENDIF

         ztrdx(:,:,:) = 0._wp  ;    ztrdy(:,:,:) = 0._wp  ;   ztrdz(:,:,:) = 0._wp
         advx(:,:,:) = 0._wp ; advy(:,:,:) = 0._wp ; advz(:,:,:) = 0._wp
      !
      zwi(:,:,:) = 0._wp
      z_rzts = 1._wp / REAL( jnzts, wp )
      zr_p2dt(:) = 1._wp / p2dt(:)
         ...
         DO jk = 1, jpkm1
            ...
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  pta(ji,jj,jk,jn) =                       pta(ji,jj,jk,jn) +         ztra   / fse3t_n(ji,jj,jk) * tmask(ji,jj,jk)
                  zwi(ji,jj,jk)    = ( fse3t_b(ji,jj,jk) * ptb(ji,jj,jk,jn) + z2dtt * ztra ) / fse3t_a(ji,jj,jk) * tmask(ji,jj,jk)
                  ! Tall addition, advection trends on i, j, k direction
                  advx(ji,jj,jk) = advx(ji,jj,jk) - (  zwx(ji,jj,jk) - zwx(ji-1,jj  ,jk  ) ) / ( e1e2t(ji,jj) * fse3t_n(ji,jj,jk) )
                  advy(ji,jj,jk) = advy(ji,jj,jk) - (  zwy(ji,jj,jk) - zwy(ji  ,jj-1,jk  ) ) / ( e1e2t(ji,jj) * fse3t_n(ji,jj,jk) )
                  advz(ji,jj,jk) = advz(ji,jj,jk) - (  zwz(ji,jj,jk) - zwz(ji  ,jj  ,jk+1) ) / ( e1e2t(ji,jj) * fse3t_n(ji,jj,jk) )
               END DO
            END DO
         END DO
         !                             ! Lateral boundary conditions on zwi  (unchanged sign)
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.  
                  ...
                  pta(ji,jj,jk,jn) = pta(ji,jj,jk,jn) + ztra
                  ! Tall addition, advection trends on i, j, k direction
                  advx(ji,jj,jk) = advx(ji,jj,jk) - zbtr * ( zwx(ji,jj,jk) - zwx(ji-1,jj  ,jk  ) )
                  advy(ji,jj,jk) = advy(ji,jj,jk) - zbtr * ( zwy(ji,jj,jk) - zwy(ji  ,jj-1,jk  ) )
                  advz(ji,jj,jk) = advz(ji,jj,jk) - zbtr * ( zwz(ji,jj,jk) - zwz(ji  ,jj  ,jk+1) )
               END DO
            END DO
         END DO

         IF( ( cdtype == 'TRC') .AND. (jn==jpo2)) THEN
            IF( iom_use("O2XADV")) THEN
               CALL iom_put("O2XADV",advx)
            ENDIF
            IF( iom_use("O2YADV")) THEN
               CALL iom_put("O2YADV",advy)
            ENDIF
            IF( iom_use("O2ZADV")) THEN
               CALL iom_put("O2ZADV",advz)
            ENDIF
            IF( iom_use("O2TADV")) THEN
               CALL iom_put("O2TADV",advx+advy+advz)
            ENDIF
         ENDIF
      ...
      !
                   ...
                   CALL wrk_dealloc( jpi, jpj, zwx_sav, zwy_sav )
                   CALL wrk_dealloc( jpi, jpj, jpk, advx, advy, advz )
      IF( l_trd )  CALL wrk_dealloc( jpi, jpj, jpk, ztrdx, ztrdy, ztrdz )
      !
      IF( nn_timing == 1 )  CALL timing_stop('tra_adv_tvd_zts')
      !
