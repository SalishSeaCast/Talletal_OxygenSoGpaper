...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: bdydta.F90 5918 2015-11-25 11:18:31Z jchanut $ 
...
902         IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambdy_dta in configuration namelist', lwp )
            ...
            IF ( cn_dyn2d(ib_bdy) == 'ssh' ) THEN
               IF ( ln_full_vel_array(ib_bdy) ) THEN
                  IF (lwp) WRITE(numout,*) "bdy_dta_init: switching ln_full_vel to false for ssh type open boundary ", ib_bdy
                  ln_full_vel_array(ib_bdy) = .FALSE.
               ENDIF
            ENDIF
