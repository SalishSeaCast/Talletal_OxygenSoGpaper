MODULE dynspg_ts
   ...
   USE stpctl          ! FD add check on depth
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) ::  sshbb_e, sshb_e, & ! Instantaneous barotropic arrays
                                                   ubb_e, ub_e,     &
                                                   vbb_e, vb_e
   LOGICAL :: ln_check_depth        ! check for negative water depth at each barotropic sub-timestep
...
CONTAINS
   ...
   SUBROUTINE dyn_spg_ts( kt, kindic )
      !!----------------------------------------------------------------------
      !!
      !! ** Purpose :   
      ...
      INTEGER, INTENT(inout)  ::   kindic ! return value > 0 if problem encountered requiring an abort action
...
!              zhf(:,:) = gdepw_0(:,:,jk+1)
      ...
      DO jn = 1, icycle                             !  sub-time-step loop  !
         ...
         IF( lk_vvl ) THEN                             !* Update ocean depth (variable volume case only)
            ...
            !
            ! global check of minimum depth
            IF( ln_check_depth ) THEN
               CALL stp_ctl_check_negative_depth( kt, kindic )
               IF( kindic == -4 ) RETURN ! bail out even if all the steps are not done
            ENDIF
            !
         ENDIF
...
CONTAINS
   INTEGER FUNCTION dyn_spg_ts_alloc()    ! Dummy function
      dyn_spg_ts_alloc = 0
   END FUNCTION dyn_spg_ts_alloc
   SUBROUTINE dyn_spg_ts( kt, kindic )    ! Empty routine
