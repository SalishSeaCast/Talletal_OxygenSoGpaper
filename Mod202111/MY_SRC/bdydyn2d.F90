MODULE bdydyn2d
   ...
   !! $Id: bdydyn2d.F90 5888 2015-11-13 16:47:32Z clem $ 
...
CONTAINS
   ...
   SUBROUTINE bdy_dyn2d( kt, pua2d, pva2d, pub2d, pvb2d, phur, phvr, pssh  )
      ...
      REAL(wp), DIMENSION(:,:), INTENT(inout) :: pssh
      !!
      ...
      DO ib_bdy=1, nb_bdy
         ...
         CASE('flather')
            CALL bdy_dyn2d_fla( idx_bdy(ib_bdy), dta_bdy(ib_bdy), rn_ssh_offset(ib_bdy), ib_bdy, pua2d, pva2d, pssh, phur, phvr )
         CASE('ssh')
            CALL bdy_dyn2d_ssh( idx_bdy(ib_bdy), dta_bdy(ib_bdy), rn_ssh_offset(ib_bdy), ib_bdy, pua2d, pva2d, pssh )
         CASE('orlanski')
   ...
   SUBROUTINE bdy_dyn2d_fla( idx, dta, ssh_offset, ib_bdy, pua2d, pva2d, pssh, phur, phvr )
      !!----------------------------------------------------------------------
      ...
      TYPE(OBC_INDEX),              INTENT(in) ::   idx  ! OBC indices
      TYPE(OBC_DATA),               INTENT(in) ::   dta  ! OBC external data
      REAL(wp),                     INTENT(in) ::   ssh_offset  ! ssh offset for boundary
      INTEGER,                      INTENT(in) ::   ib_bdy  ! BDY set index
      REAL(wp), DIMENSION(:,:), INTENT(inout) :: pua2d, pva2d
...
!!! REPLACE spgu with nemo_wrk work space
      ...
      ! Fill temporary array with ssh data + ssh_offset (here spgu):
      igrd = 1
      spgu(:,:) = 0.0
      DO jb = 1, idx%nblenrim(igrd)
         ii = idx%nbi(jb,igrd)
         ij = idx%nbj(jb,igrd)
         spgu(ii, ij) = dta%ssh(jb) + ssh_offset
      END DO
   ...
   SUBROUTINE bdy_dyn2d_ssh ( idx, dta, ssh_offset, ib_bdy, pua2d, pva2d, pssh )
      !!----------------------------------------------------------------------
      !!                 ***  SUBROUTINE bdy_dyn2d_ssh  ***
      !!
      !!              - Apply sea level time series for boundary conditions
      !!
      !! Youyu Lu  adds; October 2017
      !! Michael Dunphy tidying; May 2020
      !!----------------------------------------------------------------------
      TYPE(OBC_INDEX),              INTENT(in) ::   idx     ! OBC indices
      TYPE(OBC_DATA),               INTENT(in) ::   dta     ! OBC external data
      REAL(wp),                     INTENT(in) ::   ssh_offset  ! ssh offset for boundary
      INTEGER,                      INTENT(in) ::   ib_bdy  ! BDY set index
      REAL(wp), DIMENSION(jpi,jpj), INTENT(inout)  ::   pua2d, pva2d, pssh
      ...
      IF( nn_timing == 1 ) CALL timing_start('bdy_dyn2d_ssh')
      ...
      DO jb = 1, idx%nblenrim(igrd)
         ...
         iim1 = ii + MAX( 0, INT( flagu ) )   ! T pts i-indice inside the boundary
         iip1 = ii - MIN( 0, INT( flagu ) )   ! T pts i-indice outside the boundary
         pssh(iim1, ij) = spgu(iip1,ij)*tmask(iim1,ij,1)
         !
         iim1 = ii + INT( flagu )             ! U pts i-indice one inside the boundary
         pua2d(ii,ij) = pua2d(iim1,ij) * umask(ii,ij,1)
         !
      END DO
      !
      igrd = 3      ! BC on v-velocity
      !             ! remember that flagv=-1 if normal velocity direction is outward
      DO jb = 1, idx%nblenrim(igrd)
         ii  = idx%nbi(jb,igrd)
         ij  = idx%nbj(jb,igrd)
         ...
         ijm1 = ij + MAX( 0, INT( flagv ) )   ! T pts j-indice inside the boundary
         ijp1 = ij - MIN( 0, INT( flagv ) )   ! T pts j-indice outside the boundary
         pssh(ii, ijm1) = spgu(ii,ijp1)*tmask(ii,ijm1 ,1)
         !
         ijm1 = ij + INT( flagv )             ! V pts j-indice one inside the boundary
         pva2d(ii,ij) = pva2d(ii,ijm1) * vmask(ii,ij,1)
         !
      END DO

      CALL lbc_lnk( pua2d, 'U', -1.)
      CALL lbc_lnk( pva2d, 'V', -1.)
      CALL lbc_lnk( pssh , 'T',  1.)
      !
      IF( nn_timing == 1 ) CALL timing_stop('bdy_dyn2d_ssh')
      !
   END SUBROUTINE bdy_dyn2d_ssh
